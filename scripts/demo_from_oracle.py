"""Same pipeline as scripts/demo_from_sql.py, but fetches its input with SQL
queries against a real Oracle database (scripts/oracle_db.py) instead of an
in-memory SQLite database — run scripts/load_csv_to_oracle.py once first to
populate Oracle from data/mock/*.csv.

Usage:
    python -m scripts.demo_from_oracle
"""
import sys
...
def main() -> None:
    print("=" * 78)
    print("STEP 1/4: fetching data via SQL (real Oracle DB)")
    print("=" * 78)
    ...
    print("\n" + "=" * 78)
    print("STEP 2/4: training the LSTM (src/models/call_event_lstm.py) on Oracle-fetched data")
    print("=" * 78)
    ...
    print("\n" + "=" * 78)
    print("STEP 3/4: training the QoE regressor (src/models/qoe_regression.py) on Oracle-fetched data")
    print("=" * 78)
    ...
    out_path = DATA_DIR / "triage_queue_output_oracle.csv"
