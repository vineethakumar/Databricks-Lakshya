...
LEFT JOIN (
    SELECT site_id,
           TRUNC(alarm_ts, 'HH24')                                   AS hour_ts,
           COUNT(*)                                                  AS alarm_count,
           SUM(CASE WHEN severity = 'CRITICAL' THEN 1 ELSE 0 END)    AS critical_alarm_count
    FROM alarm_log
    GROUP BY site_id, TRUNC(alarm_ts, 'HH24')
) a ON a.site_id = cvh.site_id AND a.hour_ts = cvh.hour_ts
LEFT JOIN (
    SELECT site_id, TRUNC(event_ts, 'HH24') AS hour_ts, COUNT(*) AS event_count
    FROM network_event
    GROUP BY site_id, TRUNC(event_ts, 'HH24')
...
-- Per-site rollup of subscriber value mix — used by PKG_TRIAGE to weight
-- customer impact by how many high-ARPU / high-value subscribers a site
-- actually serves, not just raw subscriber counts.
-- ---------------------------------------------------------------------------
