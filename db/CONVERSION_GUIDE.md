...
| `FOR r IN (SELECT DISTINCT site_id ...) LOOP flag_churn_risk(r.site_id); END LOOP` | one set-based `UPDATE` correlated per site via a subquery, instead of calling the single-site procedure once per row — see the closing `UPDATE` in `build_triage_queue` |
...
| `DBMS_SCHEDULER.CREATE_JOB` (`04_triggers_scheduler.sql`) | No SQL equivalent — recreated using the Databricks SQL Editor's built-in query Schedule button (save the query, click Schedule, pick the cron time). No CLI or job-definition file needed; see the steps inside `04_triggers_scheduler.sql` |
...
- `04_triggers_scheduler.sql`: pick a SQL Warehouse when you click Schedule
  on each of the two saved queries (see the steps in that file).
